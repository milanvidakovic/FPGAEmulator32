...
public class ALU_S_REG_XX extends Instruction {
	...
	public void exec(CpuContext ctx) {
		...
		long res = 0;
		switch (type) {
		case ADD_S: res = (ctx.getReg(this.dest).val & 0xffffffffL) + (this.argument & 0xffffffffL); break;
		case SUB_S: res = (ctx.getReg(this.dest).val & 0xffffffffL) + (-this.argument & 0xffffffffL); break;
		case AND_S: res = (ctx.getReg(this.dest).val & 0xffffffffL) & (this.argument & 0xffffffffL); break;
		case OR_S : res = (ctx.getReg(this.dest).val & 0xffffffffL) | (this.argument & 0xffffffffL); break;
		case XOR_S: res = (ctx.getReg(this.dest).val & 0xffffffffL) ^ (this.argument & 0xffffffffL); break;
		case SHL_S: res = (ctx.getReg(this.dest).val & 0xffffffffL) << (this.argument & 0xffffffffL); break;
		case SHR_S: res = (ctx.getReg(this.dest).val & 0xffffffffL) >>> (this.argument & 0xffffffffL); break;
